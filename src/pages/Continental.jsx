...
const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {const fetchFoods = async () => {
    try {
      const res = await fetch(`http://localhost:4000/continental`);
      const data = await res.json();
      setFoods(data);
    } catch (error) {
      console.log('Failed to fetch continental foods')
    }
    finally {
      setLoading(false);
    }
            ...
            
              {
                foods && foods.map((item) => (
                  <div key={item.id} className="grid-item">
                    <NavLink to={`/category/continental/${item.name}`} >
